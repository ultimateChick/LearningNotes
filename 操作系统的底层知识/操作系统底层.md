cpu的计算是通过二极管的通断电运行位运算，计算机采用的是布尔体系的计算逻辑，通过与或非三种逻辑门电路进行工作，这些门电路的组合可以达成基本的加减运算等



负责控制通断电的组件叫做振荡器，也叫时间发生器，主板上的一块芯片

cpu的针脚可以一次性读进来很多01二进制数据



所谓的64位，32位的CPU就是说操作系统是以多少位为基本单位来识别电信号的，32位操作系统的内存识别很小就是因为地址空间在逻辑上耗尽了。



内存本质是存储着很多电信号，通过总线和cpu相连接



计算机通电->CPU读取内存中程序（电信号）->时钟发生器不断振动通电->推动CPU内部一步步执行(执行多少步取决于指令需要的时钟周期)->计算完成->写回(电信号)->写给显卡输出(sout/或者图形)。。



内存和硬盘的交互可以不需要经过CPU，内存通过DMA直接要求从硬盘中装载指定数据。
...
分配资源最重要的是：独立的内存空间，线程调度执行（线程共享进程的内存空间，没有自己独立的内存空间）；可以说线程就是进程当中的不同执行路线。

在linux操作系统中，线程的实现和进程没有多大区别，也是起一个普通的进程，只是这些进程都共享一块内存空间。这些进程就是所谓的Light Weight Process LWP，轻量级进程。



纤程：可以称之为线程中的线程，它是运行在用户态的，不需要进入操作系统内核态进行调度，轻量级的线程。



优势：

1：占有资源很少 线程：1M Fiber：4K 

2：切换比较简单 （不需要用户态和内核态的切换
...
子进程执行完了，父进程没有回收子进程的PCB，子进程就成了僵尸
...
子进程结束以前，父进程已经退出了，那么子进程就成了孤儿进程

在命令行界面下，孤儿进程统一由init 1号进程管理
...
Linux2.5用经典Unix O1调度策略，偏向服务器，但对交互不友好（经典时间分片，分配给不同的任务，不管任务是否紧急）

Linux2.6采用CFS调度策略：Completely Fair Scheduler，完全公平调度算法，不再采用经典的绝对时间片
...
实时进程 > 普通进程  实时进程内部分成(0~99)一百个优先级

普通进程的优先级叫做nice值（-20~19） 
...
优先级有高低，使用： - FIFO (First In First Out)

如果优先级一样 使用：- RR（Round Robin） 

普通： CFS

# 中断

![image-20200809190122168](C:\Users\q1367\Desktop\操作系统的底层知识\images\image-20200809190122168.png)

OS在内存中的特定地址存放有响应中断，发送给kernel的小程序

硬中断->中断控制器->芯片->kernel->中断处理程序->上/下半场

硬件跟操作系统内核打交道的一种机制



软中断本质就是一个特殊的中断符号，在中断向量表中，叫80H

软件进行系统调用的时候就需要进行软中断，才能进入内核态

软中断（0x80号中断） ==  系统调用



系统调用：int 0x80（C语言函数） 或者 sysenter原语（硬件支持的原语）

通过ax寄存器填入调用号（每个系统调用都有对应的编号）

参数通过bx cx dx si di传入内核（因此系统函数最多就有五个参数）

返回值通过ax返回（系统函数返回值一般都是个int类型值）

 

java读网络 – jvm read() – c库read() - > 

软中断进入内核空间 -> system_call() （系统调用处理程序）-> sys_read()
...
![image-20200809232412014](C:\Users\q1367\Desktop\操作系统的底层知识\images\image-20200809232412014.png)
...
步骤：程序加载进来，会先把页表加载进来，通知进程调度器程序入口的具体页
